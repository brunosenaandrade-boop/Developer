...
const generateResponse = (userInput: string, messageCount: number): string => {
  ...
  if (
    input.includes("site") ||
    input.includes("landing") ||
    input.includes("página") ||
  ...
  if (
    input.includes("sistema") ||
    input.includes("plataforma") ||
    input.includes("dashboard") ||
    input.includes("crm") ||
    input.includes("erp") ||
    input.includes("gestão") ||
    input.includes("gestao")
  ) {
  ...
  if (
    ...
    input.includes("vender online") ||
    input.includes("vendas")
  ) {
  ...
  if (
    input.includes("quem") ||
    input.includes("vocês") ||
    input.includes("empresa") ||
    input.includes("bruno") ||
  ...
  if (
    input.includes("onde") ||
    input.includes("endereço") ||
    input.includes("endereco") ||
    input.includes("cidade") ||
  ...
  if (
    input.includes("portfólio") ||
    input.includes("portfolio") ||
    input.includes("trabalhos") ||
    input.includes("projetos") ||
    input.includes("exemplos")
  ) {
  ...
  if (
    input.includes("sim") ||
    input.includes("quero") ||
    input.includes("pode") ||
    input.includes("vamos") ||
    input.includes("bora") ||
    input.includes("interessado") ||
    input.includes("gostaria")
  ) {
  ...
  if (
    input.includes("não sei") ||
    input.includes("nao sei") ||
    input.includes("dúvida") ||
    input.includes("duvida") ||
    input.includes("ajuda") ||
    input.includes("preciso")
  ) {
  ...
  if (
    ...
    input.includes("obrigada") ||
    input.includes("valeu") ||
    input.includes("agradeço")
  ) {
      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.button
            ...
            onClick={handleOpen}
            className="fixed bottom-24 right-6 z-50 flex items-center gap-2 bg-[#0a0a0a] border border-white/10 text-white px-4 py-3 rounded-full shadow-lg hover:border-white/30 transition-all"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 w-[calc(100vw-3rem)] sm:w-[380px] h-[500px] bg-[#030303] rounded-2xl shadow-2xl border border-white/10 flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="bg-[#0a0a0a] border-b border-white/5 px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-white text-black flex items-center justify-center font-display font-bold text-sm">
                  A
                </div>
                <div>
            ...
            {messages.length > 2 && (
              <div className="px-4 py-2 border-t border-white/5">
                <button
                  ...
                  className="w-full btn btn-whatsapp text-xs group"
                >
            ...
            <div className="p-4 border-t border-white/5">
              <div className="flex gap-2">
                <input
                  ...
                  disabled={isTyping}
                  className="flex-1 bg-[#0a0a0a] border border-white/10 rounded-full px-4 py-2.5 text-sm text-white placeholder:text-neutral-600 focus:outline-none focus:border-white/30 transition-colors disabled:opacity-50"
                />
                <button
                  onClick={sendMessage}
                  disabled={!input.trim() || isTyping}
                  className="bg-white text-black w-10 h-10 rounded-full flex items-center justify-center hover:bg-neutral-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <Send size={16} />
                </button>
              </div>
